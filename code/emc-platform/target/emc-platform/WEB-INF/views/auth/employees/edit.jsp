...
<div class="wrapper wrapper-content">
    <div class="row">
        <div class="col-sm-12">
            <form class="form-horizontal" id="employee_edit_Form" role="form">
                <input id="orgId" name="orgId" value="${object.orgId}" hidden="hidden"/>
                <input type="hidden" name="id" value="${object.id}">
                <input type="hidden" id="oldName" value="${object.empName}">
                <input type="hidden" id="oldNum" value="${object.jobNum}">
...
<script>
    ...
    //以下为官方示例
    $(function () {
        var $form =  $(top.document).find("#employee_edit_Form");
        ...
        $.validator.addMethod("checkUnique", function(value, element) {
            ...
            var newName = $(top.document).find("#empName").val();
            if(oldName == newName){
                deferred.resolve();
            }else{
                $.ajax({
        ...
        $.validator.addMethod("checkCodeUnique", function(value, element) {
            var deferred = $.Deferred();//创建一个延迟对象
            var deferred = $.Deferred();//创建一个延迟对象
            var oldNum = $(top.document).find("#oldNum").val();
            var newNum = $(top.document).find("#jobNum").val();
            if(oldNum == newNum){
                deferred.resolve();
            }else {
                $.ajax({
