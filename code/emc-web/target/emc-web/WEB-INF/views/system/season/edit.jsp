        ...
        <div class="col-sm-12 col-xs-12 col-md-12 col-lg-12">
            <form class="form-horizontal" id="seasonEditForm" rseasonAddFormole="form">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input id="start1" type="text" value="${season.sdate}" class="laydate-icon  form-control layer-date"  name="sDate"
                               placeholder="请选择开始时间"/>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input id="end1" type="text" value="${season.edate}" class="laydate-icon  form-control layer-date"  name="eDate"
                               placeholder="请选择结束时间"/>
                    </div>
                </div>
    ...
    $(function () {
        ...
        $.validator.addMethod("checkSeason", function (value, element) {
            ...
            if("${season.name}" == name){
                ...
                $.ajax({
                    ...
                    data: {name: value,comId:'${season.comid}'},
                    dataType: 'json',
        ...
        $.validator.addMethod("isAmong", function (value, element, param) {
            ...
            if("${season.edate}" == value&&"${season.sdate}"==start){
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    async: false,//要指定不能异步,必须等待后台服务校验完成再执行后续代码
                    data: {sdate: start, edate: value,comId:'${season.comid}'},
                    dataType: 'json',
                    success: function (result) {
                        if (result.flag) {
                            deferred.reject();
        ...
        $form.validate({
            ...
            submitHandler: function () {
                ...
                $.ajax({
                    url: _web + '/season/editvalue',
                    data: {
                        id:$(top.document).find('#id').val(),
                        name:$(top.document).find("#season").val(),
                        sdate:$(top.document).find('#start1').val(),
                        edate:$(top.document).find('#end1').val()
                    },
                    type: 'POST',
                    dataType: 'json',
                    success: function (result) {
